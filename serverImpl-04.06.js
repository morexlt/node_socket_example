...
server.on('connection', function(sock){
	...
	sock.on('data', function(data){
		...
		if(data.substring(0,7) == "#nombre"){
			console.log("VA A PONER EL NOMBRE");
			console.log("El nombre es: " + data.substring(8,data.length));
			sock.nombre = data.substring(8,data.length);
		}
		if(sock.num == 0){
			//sock.nombre = data;
			sock.num = 1;
			...
			users.forEach(function(element){
				console.log('users');
				//console.log(element);sock.remotePort
				console.log('users');
				if(!(element.remoteAddress == sock.remoteAddress && element.remotePort == sock.remotePort)){
					console.log('asd');
					element.write(sock.nombre.substring(0,sock.nombre.length-1) + ' dijo: ' + data);
				}
